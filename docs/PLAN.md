...
| Загрузчик / root | **разблокирован** (2026-09-25) / нет. BROM доступен: SBC/SLA/DAA включены, но `mtkclient` обходит их (Kamakiri) — запасной путь разблокировки без ожидания |
...
| S2 | Разблокировка загрузчика → Magisk → проверка root | владелец + агент | 🟡 2026-09-25: загрузчик разблокирован через Mi Unlock 7.6.727.43 **сразу, без таймера** (данные стёрты). Далее — Magisk |
