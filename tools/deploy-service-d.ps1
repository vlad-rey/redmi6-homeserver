# Устанавливает скрипты из device\service.d в /data/adb/service.d на телефоне (нужен root).
# Запуск: powershell -ExecutionPolicy Bypass -File tools\deploy-service-d.ps1 [-Serial <IP>:5555]
...
foreach ($f in Get-ChildItem $Src -Filter *.sh) {
    # Гарантируем LF-переводы строк, иначе sh на телефоне не выполнит скрипт
